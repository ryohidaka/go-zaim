...
func TestDeletePayment(t *testing.T) {
	...
	t.Run("正常系: 支払情報を正しく削除できる", func(t *testing.T) {
		// 正常なレスポンスを返すモックを設定
		err := testutil.MockResponseFromFile("DELETE", zaim.BaseURL+"home/money/payment/11820767", "transaction")
		assert.NoError(t, err)

		// 支払情報を削除
		res, err := c.DeletePayment(11820767)
	...
	t.Run("異常系: サーバーエラー時にエラーを返却する", func(t *testing.T) {
		// エラーを返すモックに差し替え
		httpmock.RegisterResponder("DELETE", zaim.BaseURL+"home/money/payment/11820767",
			httpmock.NewStringResponder(500, `Internal Server Error`))
